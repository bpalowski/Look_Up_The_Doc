    ...
    function con(user){
      if(user.data.length == 0){
        ...
        for(let f= 0; f < user.data.length; f++){
          $('#showDetails').prepend(`<li> Specialities: ${user.data[f].specialties[0].uid} </li><li>${user.data[f].specialties[0].description}</li><li>Name: ${user.data[f].profile.first_name} ${user.data[f].profile.last_name}</li><li>Address: ${user.data[f].practices[0].visit_address.city} ${user.data[f].practices[0].visit_address.state} </li><li>Phone: ${user.data[f].practices[0].phones[0].number}</li>`);
          console.log(user);
        }
      }
    }
